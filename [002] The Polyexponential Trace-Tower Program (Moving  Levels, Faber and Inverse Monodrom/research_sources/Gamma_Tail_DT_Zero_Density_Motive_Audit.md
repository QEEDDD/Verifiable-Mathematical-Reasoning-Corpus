# Fixed-cut incomplete Gamma tail: differential transcendence, zero density, and the Euler-motive barrier

## Status and scope

This note audits consequences of the all-order value theorem in **[001]** together with the universal inverse-trace identities in **Moving Level Trace Towers v2.3**.  It does not modify either manuscript.  Every unconditional statement below is conditional only on the theorems already proved in [001]/v2.3; claims about \(\gamma\), \(\delta\), or comparison injectivity are explicitly separated from collective tail statements.
...
\[
 c_k:=g_k-T_k=(-1)^{k+1}\Ein_{k+1}(1),                         \tag{1}
\]
...
## 1. Exact finite gamma-jet field and the tail transcendence-degree bound

### Lemma 1 (exact gamma-jet field)
...
\[
 ...
 \zeta\!\left(2\left\lfloor\frac N2\right\rfloor+1\right)\right).}       \tag{2}
\]
...
\[
 ...
 \le 2+\left\lfloor\frac N2\right\rfloor \qquad(N\ge1).                 \tag{3}
\]
...
### Theorem 2 (half-rank lower bound for the fixed-cut tail)
...
\[
 ...
 \ge \left\lceil\frac N2\right\rceil-1.}                                  \tag{4}
\]

For \(N=0\), the corresponding lower bound is \(0\).  In particular,

\[
 \operatorname{trdeg}_K K(T_0,T_1,\ldots)=\infty.                          \tag{5}
\]
...
indices in the weak counting sense supplied by (4), but it does **not** name
\(T_0=\delta/e\) as one of them.

---

## 2. Mahler's coefficient-field theorem and a fixed-cut DT theorem

### The exact Mahler input

The needed result is the formal-power-series theorem in K. Mahler,
“A remark on algebraic differential equations,” *Rend. Accad. Naz. Lincei*,
Ser. VIII **50** (1971), 402–412, especially §§3–5
([scan](https://carmamaths.org/resources/mahler/docs/176.pdf)).

> **Mahler's theorem (form used here).** Let \(L\) be any field of
...
\[
 A(n)f_{n+1}=B_n(f_0,\ldots,f_n),                                           \tag{6}
\]
...
### Theorem 3 (fixed-cut upper incomplete Gamma is differentially transcendental)
...
\[
 \boxed{U(s)\text{ is differentially transcendental over }\mathbf C(s).}    \tag{7}
\]
...
### Spectral formal-series corollary
...
\[
 ...
 =\operatorname{trdeg}_K K(T_k:k\in I),                                    \tag{9}
\]
...
### Exact overlap with Arreche (2013)

C. E. Arreche, “A Galois-theoretic proof of the differential transcendence
of the incomplete Gamma function,” *J. Algebra* **389** (2013), 119–127
([arXiv:1304.1917](https://arxiv.org/abs/1304.1917)), proves

\[
 \gamma(t,x)=\int_0^x u^{t-1}e^{-u}\,du
 \quad\text{is }\partial_t\text{-transcendental over }\mathbf C(t,x).       \tag{10}
\]
...
\[
 \partial_x^2Y-\frac{t-1-x}{x}\partial_xY=0.                               \tag{11}
\]
...
* Both results assert differential transcendence in the **parameter/order**
  direction.
...
* The bivariate assertion (10) does **not formally specialize** to the
...
## 3. A half-density theorem for wholly transcendental tail divisors
...
### Lemma 5 (only possible algebraic zero)
...
### Theorem 6 (half-density all-zero theorem)
...
\[
 \boxed{A_N\le2+\left\lfloor\frac N2\right\rfloor.}                         \tag{13}
\]

Consequently at least

\[
 \boxed{\left\lceil\frac N2\right\rceil-1}                                 \tag{14}
\]

of the divisors \(F_0,\ldots,F_N\) have **all** their zeros transcendental.
In density notation,
...
1. every zero of \(\Ein(z)-E_1(1)\) is transcendental;
2. \(r_0\) is algebraic, and \(\gamma\) and \(\delta\) are algebraically
   independent over \(K\).
...
### An Euler–Gompertz pencil strengthening

For real algebraic \(t\in[0,1]\), put

\[
 c_t:=(1-t)\gamma+tE,
 \qquad \Ein(q_t)=c_t,\quad q_t\in(0,1).                                    \tag{16}
\]
...
\[
 (t-s)\eta+(1-2t)c_s-(1-2s)c_t=0.                                          \tag{17}
\]
...
## 4. What the inverse traces do and do not add to the Euler motive
...
\[
 c^{-1}=144\mathcal T_4(c)-144\mathcal T_2(c)^2-14\mathcal T_2(c).          \tag{18}
\]

### Exact spectral reformulations
...
\[
 ...
 =\operatorname{trdeg}_K K(\gamma,\delta)\in\{1,2\}.}                     \tag{19}
\]
...
\[
 ...
 \qquad a\mapsto2\pi i,\quad b\mapsto\gamma,                              \tag{20}
\]
...
\[
 ...
 \overline{\mathbf Q}(2\pi i,\gamma)=2.                                   \tag{21}
\]
...
\[
 ...
 \iff
 \operatorname{trdeg}_{\overline{\mathbf Q}}
 \overline{\mathbf Q}(2\pi i,S_2,S_4)=2.}                                 \tag{22}
\]
...
1. **Global DT does not isolate the constant term.**  Equation (7) only says
...
2. **The gamma-jet upper field has enough unknown capacity.**  The odd
...
3. **Inverse traces are not automatically motivic periods.**  The period
...
4. **Degree-one fullness is already the hard \(\gamma\) problem.**  The
...
5. **Bare Frobenius does not retain the extension.**  The unfiltered
...
6. **Function-level differential Galois rigidity is a different axis.**
...
### One genuine motivic-size consequence
...
## 5. Bottom-line classification

| Claim | Status from [001] + v2.3 | Arithmetic force on \(\gamma,\delta\) |
|---|---:|---:|
...
The strongest genuinely new-looking transfer is therefore the combination
of (i) a **fixed-cut differential-transcendence theorem**, (ii) a
**half-density all-zero theorem**, and (iii) sharp **conditional zero/trace
